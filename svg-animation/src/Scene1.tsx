...
const Tears: React.FC<{ frame: number; x: number; y: number; delay: number }> = ({ frame, x, y, delay }) => {
  const tearProgress = ((frame + delay) % 50) / 50;
  const tearY = interpolate(tearProgress, [0, 1], [y, y + 60]);
...
export const Scene1: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* ========================================================
          HUGO GROTIUS (PRISONER WITH TRUMP FEATURES FOR TESTING)
          ======================================================== */}
      <g
        style={{
      ...
      >
        <g style={{ transform: `translateY(${hugoBob}px)` }}>
          ...
          {/* Trump Red Tie & Suit Collar (Drawn on the body) */}
          {/* White Collar flaps */}
          <polygon points="540,335 550,345 540,352" fill="white" stroke="black" strokeWidth="1.5" />
          <polygon points="560,335 550,345 560,352" fill="white" stroke="black" strokeWidth="1.5" />
          {/* Red Tie */}
          <polygon points="547,344 553,344 556,380 550,390 544,380" fill="#e03131" stroke="black" strokeWidth="2" />
          ...
          {/* Head Fills to merge outlines */}
          <circle cx="502" cy="305" r="32" fill="white" />
          <circle cx="598" cy="305" r="32" fill="white" />
          <circle cx="550" cy="295" r="44" fill="white" />

          {/* Trump Swoop Blonde Hair (Drawn on top of the head) */}
          <path
            d="M 500 270 C 490 215, 595 210, 610 260 C 590 275, 570 265, 550 272 C 530 265, 515 275, 500 270 Z"
            fill="#ffd43b" // Trump signature blonde yellow
            stroke="black"
            strokeWidth="3.5"
            strokeLinejoin="round"
          />

          {/* Face details (Sad/Crying) */}
          ...
          <Tears frame={frame} x={568} y={296} delay={20} />
        </g>
      </g>
